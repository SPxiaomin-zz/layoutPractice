...
课程链接： <http://www.imooc.com/learn/100>
...
- 细节分析
    ...
    - li 元素中的子元素都是浮动的元素，所以 li 元素通过设置高度的方式来消除浮动，实在也是非常的巧妙的一种方式；
    - .reg_item 元素通过 `line-height` 和 `text-align` 实现了垂直方向和水平方向上的布局；
    - 下面的 .autoLogin 和 btn 都是通过左边添加一个空的 span 元素实现和上面的元素对齐的；
